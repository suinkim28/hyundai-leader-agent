...
def graph_credentials_missing():
    ...
    시연할 수 없고, 부트스트랩의 마지막 단계가 빈손이 된다.
    """
    try:
...
def main():
    ...
    if profile_state in ("미완료", "없음"):
        emit(
            ...
            "이 세션에서 사용자가 무엇을 요청하든, 먼저 BOOTSTRAP.md 1단계를 따르십시오. "
            "일반적인 조언으로 답하지 마십시오.\n"
            "순서: attachments/raw/ 와 knowledge_base/ 의 사전자료를 먼저 읽어 PROFILE.md 를 "
            "채우고 5줄로 보여드립니다 → 자료로 확인되지 않는 것만 한 번에 하나씩 여쭙습니다 "
            "→ 답을 들을 때마다 PROFILE.md 를 즉시 갱신합니다. 필수 항목이 차면 상태를 "
            "`진행중` 으로 바꾸고 Quick Win 한 건을 실제로 실행해 보여 주십시오."
        )
        return 0
