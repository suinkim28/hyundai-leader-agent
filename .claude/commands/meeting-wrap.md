...
python3 scripts/transcribe_audio.py <오디오경로> \
  --output meetings/transcripts/$(date +%F)_<제목>_TRANSCRIPT.md \
  --json-output meetings/transcripts/$(date +%F)_<제목>_TRANSCRIPT.json
```
...
`meetings/logs/$(date +%F)_<제목>_LOG.md` 로 저장한다.
