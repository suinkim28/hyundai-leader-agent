...
**이 문서의 전제 하나:** 승인은 늦게 나온다. 그래서 이 에이전트는
**연결이 하나도 없어도 동작하도록** 설계되어 있다. 연결은 기능을 늘리는
것이지 동작의 전제가 아니다.
...
3. 첫 로그인을 한 번 수행한다. 이후 refresh token 이 자동 갱신된다.
   ```bash
   python3 scripts/fetch_outlook_mail.py --top 1
   ```
...
python3 scripts/fetch_outlook_mail.py --search "키워드" --top 10
```
...
python3 scripts/transcribe_audio.py <오디오> --output meetings/transcripts/...
```
