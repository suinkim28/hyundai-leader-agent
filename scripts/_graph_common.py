...
import 하지 않는다 (순환 참조 방지).

2026-09-09: 원래 이 인증/토큰/시크릿 코드는 `fetch_teams_message.py` 에
있었다. Teams 조회 기능을 제일 먼저 만들 때 인증 코드를 같이 짰고, 이후
메일/캘린더/파일/노트 스크립트가 늘어날 때마다 "이미 있는 인증 코드"를 거기서
`import` 해다 쓰면서 굳어진 구조였다. 이름과 실제 역할이 뒤바뀌어 있어서
(Teams 전용 스크립트가 전체의 인증 기반이 되어 있었음) 여기로 옮겼다.
`fetch_teams_message.py` 는 이제 이름 그대로 Teams 메시지 조회 CLI 로직만
가지고 있고, 이 파일에서 인증 관련 이름을 import 해 쓴다.
"""
