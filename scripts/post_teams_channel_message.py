#!/usr/bin/env python3
"""Post a new Microsoft Teams channel message from a channel deep link via Graph API."""
...
def parse_channel_url(url: str) -> dict[str, str]:
    ...
    if len(parts) < 5 or parts[1:3] != ["l", "channel"]:
        raise ValueError("Unsupported Teams channel URL format")

    channel_id = urllib.parse.unquote(parts[3])
    channel_name = urllib.parse.unquote(parts[4])
    team_id = query.get("groupId", [""])[0]
    tenant_id = query.get("tenantId", [""])[0]
    if not channel_id or not team_id:
        raise ValueError("Missing team/channel identifiers in URL")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Post a new Teams channel message from a channel deep link.")
    parser.add_argument("url", help="Teams channel deep link")
...
def main() -> int:
    ...
    try:
        load_dotenv()
        source = parse_channel_url(args.url)
        message = read_body(args)
        encoded_channel_id = urllib.parse.quote(source["channel_id"], safe="")
        url = f"{GRAPH_BASE}/teams/{source['team_id']}/channels/{encoded_channel_id}/messages"
        ...
        print(f"Sent Teams channel message: {result.get('id')}")
